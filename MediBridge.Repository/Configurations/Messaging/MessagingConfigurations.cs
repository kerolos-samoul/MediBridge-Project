...
public sealed class DoctorMessageQueueConfiguration : IEntityTypeConfiguration<DoctorMessageQueue>
{
    public void Configure(EntityTypeBuilder<DoctorMessageQueue> builder)
    {
        ...
        builder.HasIndex(queue => new { queue.DoctorId, queue.Status, queue.CampaignSubmittedAtUtc, queue.QueuedAtUtc, queue.Id });
        builder.HasIndex(queue => new { queue.CampaignId, queue.DoctorId })
            .IsUnique()
